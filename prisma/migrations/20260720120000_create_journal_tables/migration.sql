...
-- Enums (DO blocks pour compatibilité PostgreSQL < 14 qui ne supporte pas IF NOT EXISTS)
DO $$ BEGIN IF NOT EXISTS (SELECT 1 FROM pg_type WHERE typname = 'Direction') THEN CREATE TYPE "Direction" AS ENUM ('BUY', 'SELL'); END IF; END $$;
DO $$ BEGIN IF NOT EXISTS (SELECT 1 FROM pg_type WHERE typname = 'TradeResult') THEN CREATE TYPE "TradeResult" AS ENUM ('WIN', 'LOSS', 'BREAKEVEN'); END IF; END $$;
DO $$ BEGIN IF NOT EXISTS (SELECT 1 FROM pg_type WHERE typname = 'Mood') THEN CREATE TYPE "Mood" AS ENUM ('CONFIDENT', 'NEUTRAL', 'ANXIOUS', 'FEARFUL', 'GREEDY', 'REVENGE'); END IF; END $$;
DO $$ BEGIN IF NOT EXISTS (SELECT 1 FROM pg_type WHERE typname = 'StreakType') THEN CREATE TYPE "StreakType" AS ENUM ('WIN_STREAK', 'LOSS_STREAK', 'DISCIPLINE_STREAK'); END IF; END $$;
DO $$ BEGIN IF NOT EXISTS (SELECT 1 FROM pg_type WHERE typname = 'TradeStrategy') THEN CREATE TYPE "TradeStrategy" AS ENUM ('SCALPING', 'DAY_TRADING', 'SWING', 'POSITION'); END IF; END $$;
DO $$ BEGIN IF NOT EXISTS (SELECT 1 FROM pg_type WHERE typname = 'TradeSetup') THEN CREATE TYPE "TradeSetup" AS ENUM ('BREAKOUT', 'PULLBACK', 'REVERSAL', 'RANGE', 'TREND', 'OTHER'); END IF; END $$;
