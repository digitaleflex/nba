...
const mockPlans = [
  ...
  { id: "silver", name: "Plan Silver", description: "Intermédiaire", sortOrder: 2 },
  { id: "gold", name: "Plan Gold", description: "Premium", sortOrder: 3 },
]

function mockFetchSuccess() {
  return vi.spyOn(global, "fetch").mockImplementation(async (url: string | URL | Request) => {
...
function getPasswordToggle(passwordInput: HTMLElement) {
  return passwordInput.closest(".relative")?.querySelector("button") as HTMLElement
}
...
async function fillWizard(user: ReturnType<typeof userEvent.setup>) {
  ...
  await user.type(screen.getByPlaceholderText("a.mensah@exemple.com"), "kofi@test.com")
  await user.type(screen.getByPlaceholderText("+229 01 02 03 05 06"), "+22901020305")
  await user.click(screen.getByRole("button", { name: /suivant/i }))
  await user.type(screen.getByPlaceholderText("Min. 8 caractères"), "Str0ng!Pass")
  await user.click(screen.getByRole("button", { name: /suivant/i }))
}
...
describe("Register Wizard", () => {
  ...
  it("loads plans and shows options on combobox click", async () => {
    ...
    await user.click(screen.getByRole("combobox"))
    expect(await screen.findByRole("option", { name: "Plan Bronze" })).toBeInTheDocument()
    expect(screen.getByRole("option", { name: "Plan Silver" })).toBeInTheDocument()
    expect(screen.getByRole("option", { name: "Plan Gold" })).toBeInTheDocument()
  })
  ...
  it("navigates from service selection to identity step", async () => {
    ...
    const user = userEvent.setup()
    await user.click(screen.getByRole("combobox"))
    await user.click(await screen.findByRole("option", { name: "Plan Silver" }))
    await user.click(screen.getByRole("button", { name: /continuer/i }))
    expect(screen.getByText(/Commençons par votre identité/i)).toBeInTheDocument()
  })
  ...
  it("displays password strength rules on typing", async () => {
    ...
    await user.type(screen.getByPlaceholderText("Mensah"), "Mensah")
    await user.click(screen.getByRole("button", { name: /suivant/i }))
    await user.type(screen.getByPlaceholderText("a.mensah@exemple.com"), "k@t.com")
    await user.type(screen.getByPlaceholderText("+229 01 02 03 05 06"), "+22901020305")
    await user.click(screen.getByRole("button", { name: /suivant/i }))
  ...
  it("toggles password visibility", async () => {
    ...
    await user.type(screen.getByPlaceholderText("+229 01 02 03 05 06"), "+22901020305")
    await user.click(screen.getByRole("button", { name: /suivant/i }))
    const input = screen.getByPlaceholderText("Min. 8 caractères") as HTMLInputElement
  ...
  it("shows confirmation step with all entered data", async () => {
    ...
    const reviewSection = screen.getByText(/Vérifiez vos informations/i).parentElement!
  ...
  it("clears sessionStorage on successful registration", async () => {
    ...
    const remainingKeys = Object.keys(sessionStorage).filter(k => k.startsWith("nba_register"))
