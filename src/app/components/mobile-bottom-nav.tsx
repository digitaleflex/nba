...
export function MobileBottomNav({ isAdmin = false, user }: MobileBottomNavProps) {
  ...
  const userLinks: MobileNavLink[] = [
    ...
    },
    {
      ...
      label: "Onboarding",
      icon: Shield,
      active: pathname.startsWith("/dashboard/verification") || pathname.startsWith("/onboarding"),
    },
      ...
      <div className="md:hidden fixed bottom-0 left-0 right-0 z-50 border-t bg-card/85 backdrop-blur-lg px-1 select-none" style={{ height: "calc(4rem + env(safe-area-inset-bottom, 0px))", paddingBottom: "env(safe-area-inset-bottom, 0px)" }}>
        <nav className="flex h-16 items-center justify-around">
          {links.map((link, idx) => {
            ...
            const showAccesBadge = isAdmin && link.href === "/admin?tab=requests" && pendingRequests > 0
