...
import { Video, Upload, ArrowRight, Loader2 } from "lucide-react"
...
interface BrokerVerificationFormProps {
  onSubmit: (form: FormData) => Promise<{ ok: boolean; error?: string }>
  onSuccess: () => void
  initialBrokerName?: string
  initialAccountId?: string
  animated?: boolean
  onStateChange?: (state: { brokerName: string; accountId: string }) => void
}

export function BrokerVerificationForm({
  onSubmit,
  onSuccess,
  initialBrokerName = "",
  initialAccountId = "",
  ...
  useEffect(() => {
    ...
    timerRef.current = setTimeout(() => {
      onStateChangeRef.current?.({ brokerName, accountId })
    }, 800)
